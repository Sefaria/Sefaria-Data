...
#                  "Bikkurim"]
#
#     ref_prefix = "Lechem Shamayim on Mishna "
...
def create_text_object(lines):
    ...
    def ref_generator(masechet_index, perek, mishna, segment):
        return "Lechem Shamayim on Mishna " + masechtot[masechet_index] + " " + str(perek) + " " + str(mishna) + ":" + str(segment)
    masechet_index = -1
    perek_index = 0
    mishna_index = 0
    segment_index = 0
    ...
    for line in lines:
        if  "סליקא מסכת" in line:
            continue
        if '@00' in line and "מסכת" in line:
            masechet_index+=1
            perek_index = 0
            mishna_index = 0
            segment_index = 0
...
if __name__ == '__main__':
    print("hello world")
    file_path = "lechem_zeraim.docx"
    # Call the function to read the DOCX file
    docx_text = read_docx(file_path)
    a = parse_text(docx_text)
    b = create_text_object(a)
