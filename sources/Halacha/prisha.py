...
def build_index():
    root = SchemaNode()
    root.key = 'Prisha'
    root.add_title("Prisha", "en", primary=True)
    root.add_title(u"פרישה", "he", primary=True)
...
def open_file():
    with open("source/prishaOC1.txt", 'r') as filep:
        file_text1 = filep.read()
        ucd_text1 = unicode(file_text1, 'utf-8').strip()
    with open("source/prishaOC2.txt", 'r') as filep:
...
def save_parsed_text(parsed):
    ...
    with open("preprocess_json/Prisha.json", 'w') as out:
