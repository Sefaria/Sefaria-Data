...
def postFile2(filename, index, textIndex, symbolList, actuallyPost = True, useRealSite = False, postBookIndex = True, displayHeaderSymbols = [], createHeader = [], headerAttr = [], useAsNumberForLocation = [], postSifKatans = False):
	
	
	if((not postSifKatans and len(index["sectionNames"]) != len(symbolList)) or (postSifKatans and len(index["sectionNames"]) != len(symbolList) +1)):
