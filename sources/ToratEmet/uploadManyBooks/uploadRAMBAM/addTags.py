...
def addTags(text, keepTrack):
	...
	text = re.sub('\[\[\[.{1,3}\]\]\]', "", text) #can be used to replace [[[heberwLetter(s)]]] #text = addTag(text, "[[[", "<b>", "]]]", "</b>", keepTrack)
	text = addTag(text, "[[", "", "]]", "", False)	#text = addTag(text, "[[", "<b>", "]]", "</b>", False)
	text = addTag(text, "(((", "<small>", ")))", "</small>", keepTrack)
	text = addTag(text, "((", "<small>", "))", "</small>", False)
	text = addTag(text, "{{{{", "<small>", "}}}}", "</small>", keepTrack)
	text = addTag(text, "{{", "<small>(", "}}", ")</small>", keepTrack)	#text = addTag(text, "{{", "<small>", "}}", "</small>", keepTrack)
	text = text.replace("(-)","")#different
	text = text.replace("*","")#different
	
	#text = re.sub('\s\{.{1,3}\}', "", text) #can be used to replace {number}
	text = re.sub('\[[\u05d0-\u05ea]{1,3}\]', "", text) #can be used to replace [heberwLetter]
	text = addTag(text, "{", "\"", "}", "\"", False)		#text = addTag(text, "{", "<small>", "}", "</small>", False)
	#text = addTag(text, "(", "<small>(", ")", ")</small>", False)
	text = text.replace("  "," ")#different
