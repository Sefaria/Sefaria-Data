...
for line in f:
    ...
    if line.find("@22")>=0:
        ...
        if actual_line.find("עד")>=0:
            ...
            mitzvah_two = getGematria(line.split(" ")[len(line.split(" "))-2])
            text[mitzvah_one] = []
            text[mitzvah_two] = []
...
for mitzvah in text:
    ...
    for count, each_comment in enumerate(mitzvah_text):
        links.append({
                "refs": [
                         "Brit Moshe, Volume One."+str(mitzvah)+"."+str(count+1),
                        "Sefer Mitzvot Gadol, Volume One."+str(mitzvah)+".1"
                    ],
                "type": "commentary",
                "auto": True,
