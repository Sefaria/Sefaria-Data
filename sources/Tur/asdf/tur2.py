...
def replaceWithHTMLTags(text, helek, siman_num):
	commentaries = ["Drisha", "Prisha", "Darchei Moshe", "Hagahot", "Beit Yosef", "Bach", "Mystery"]
	matches_array = [re.findall("\[.*?\]",text), re.findall("\([^%#].*?\)",text), re.findall("\(\%.*?\)",text), 
						re.findall("#.{1,2}", text), re.findall("\{.*?\}",text), re.findall("\|.*?\|", text),
						re.findall("<.*?>",text)]
	for commentary_count, matches in enumerate(matches_array):
		for order_count, match in enumerate(matches):
			if commentaries[commentary_count] == "Prisha" and helek == "Orach Chaim":
				pdb.set_trace()
			HTML_tag =  '<i data-commentator="'+commentaries[commentary_count]+'" data-order="'+str(order_count+1)+'"></i>'
			text = text.replace(match, HTML_tag)
		csvwriter.writerow([helek, commentaries[commentary_count], siman_num, len(matches)])
	return text			
...
def parse_text(at_66, at_77, at_88, helekim, files_helekim):
  for count, helek in enumerate(helekim):
	...
	for line in f:
		...
		if len(line)==0:
			continue
		if line.find("@")==line.rfind("@") and (line.find("@22")>=0 or line.find("@00")>=0):
			line = line.replace("@33","").replace("@11","").replace("@22","").replace("@00","").replace("@66","")
			hilchot_topic[helek][line] = []
			hilchot_topic[helek][line].append(this_siman+1)
			if len(prev_hilchot_topic)>0:
				hilchot_topic[helek][prev_hilchot_topic].append(this_siman)
			prev_hilchot_topic = line
			if line.find("@")>=0:
				pdb.set_trace()
		else:
			...
			first_word_88 = -1
			siman_header = False
			if line.find("@22")>=0 or line.find("@00")>=0:
				siman_header = True
			...
			if line.find("@")>=0:
				pdb.set_trace()
			if siman_header:
				text[helek][int(current_siman)] = [line]
			else:
				text[helek][int(current_siman)][0] = text[helek][int(current_siman)][0]+"<br>"+line
  hilchot_topic[helek][prev_hilchot_topic].append(this_siman)
...
if __name__ == "__main__":
	...
	at_77 = " || "
	at_88 = " <> "
	files_helekim = ["yoreh deah/tur.txt", "choshen mishpat/tur.txt",
   "even haezer/tur.txt", "orach chaim/tur.txt"]
