...
def replaceWithHTMLTags(text, helek, siman_num):
	...
	matches_array = [re.findall("\[.*?\]",text), re.findall("\([^.*%#]{1,4}\)",text), re.findall("\(\%.*?\)",text), 
						re.findall("#.{1,4}", text), re.findall("\{.*?\}",text), re.findall("\|.*?\|", text),
						re.findall("<.*?>",text)]
...
def parse_text(at_66, at_77, at_88, helekim, files_helekim):
  for count, helek in enumerate(helekim):
	...
	text[helek] = {}
	for line in f:
		if line.find("@22דברים ")>=0:
			pdb.set_trace()
		actual_line = line
		line = line.replace('\n','')
		if len(line)==0:
			continue
		if line.find("@")==line.rfind("@") and (line.find("@22")>=0 or line.find("@00")>=0) and len(line)>8:
			...
			line = line.replace("@33","").replace("@11","").replace("@00","").replace("@44","").replace("@55","").replace("@99","").replace("@98","").replace("@89","")
