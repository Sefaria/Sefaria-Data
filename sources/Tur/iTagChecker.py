...
class iTagChecker:
	...
	def __init__(self, bookRef, commentators, refReplace):
		...
		for count, commentator in enumerate(commentators):
			self.commentator_refs[commentator] = bookRefStr.replace(refReplace, commentator)
	...
	def output(self):
		...
		reportFile = open("big_probs_number_off_"+self.bookRef.normal()+".txt", 'a')
	...
	def checkSection(self, section):
		...
		for commentator in commentators:
			...
			if len(commentator_ref.text("he").text) == 0: #ref not valid
				...
				how_many_comments = len(commentator_ref.all_subrefs())
...
if __name__ == "__main__":
	refs = [Ref("Tur, Orach Chaim"), Ref("Tur, Yoreh Deah"), Ref("Tur, Even HaEzer"), Ref("Tur, Choshen Mishpat")]
	commentators = ["Bach", "Beit Yosef", "Prisha", "Drisha", "Darchei Moshe"]
	for ref in refs:
		print ref.normal()
		checker = iTagChecker(ref, commentators, "Tur")
		checker.run()
		checker.output()
