...
def checkCSV(helek, commentator, siman, num_comments, prev_line):
	...
	first_words = " ".join(prev_line.split(" ")[0:2])
	for row in csvreader:
		if helek == row[0] and commentator == row[1] and str(siman) == row[2]:
			...
			if abs(num_comments-int(row[3])) > 0 and abs(num_comments-int(row[3])) <= 5:
				num_comments_mismatch_small.write(helek.replace(" ","_")+";"+commentator+";Siman:"+str(siman)+";"+commentator+"_Count;"+str(num_comments)+";Tur_Count:"+row[3]+';First_Words:'+first_words+'\n')
			elif abs(num_comments-int(row[3])) > 5:
				num_comments_mismatch_big.write(helek.replace(" ","_")+";"+commentator+";Siman:"+str(siman)+";"+commentator+"_Count;"+str(num_comments)+";Tur_Count:"+row[3]+';First_Words:'+first_words+'\n')
	csvf.close()
