...
if __name__ == "__main__":
    ...
    parshiyot = []
    with open("Torah Ohr Main Text.csv", 'r') as f:
    ...
    index_dict = {"title": title, "schema": root.serialize(), "categories": ["Chasidut", "Early Works"]}
    ...
    total = len(lines)
    bad = 0
    with open("Torah Ohr Main Text with Logic Links.csv", 'w') as f:
        writer = csv.writer(f)
        for line in lines:
            if line[1] == line[2] == "":
                bad += 1
    ...
    print(total)
    #
    # with open("Likkutei Torah Main Text.csv", '