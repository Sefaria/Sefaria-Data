...
"Hilchot Sefer Torah, Sefer HaItim",
"Hilchot Mezuza",
"Hilchot Tefilin",
"Hilchot Tefilin, Shimoosha Raba",
"Hilchot Tefilin, Kitzur Tefilin",
"Hilchot Tzitzit",
...
def parse(text):
    ...
    for i,part in enumerate(text):
        ...
        for i,siman in enumerate(seifim[1:]):
            ...
            if len(cut) >2:
                shmuel.append("<b> " + re.sub('@[0-9][0-9]',"",cut[0]) + " </b> " +  re.sub('@[0-9][0-9]',"",cut[1]) +  re.sub('@[0-9][0-9]',"",cut[2]))
            elif len(cut)>1:
...
#    root.add_title("Halachot Ketanot laRosh","en", primary= False)
 #   root.add_title(u'הלכות קטנות לרא"ש',"he", primary= False)
    ...
    #sefer torah
    itim = JaggedArrayNode()
    itim.add_title("Hilchot Sefer Torah, Sefer HaItim", "en", primary=True)
    itim.add_title("ספר העתים", "he", primary=True)
    itim.key = itim.primary_title()
    itim.depth = 1
    itim.sectionNames = [ "Siman"]
    itim.addressTypes = ["Integer"]
    #Mezuza
    ...
    kitzur.add_title("Hilchot Tefilin, Kitzur Tefilin", "en", primary = True)
    kitzur.add_title("קיצור תיקון תפילין ", "he", primary = True)
    kitzur.key = shimoosha.primary_title()
...
def save_parsed_text(text, record, part="" ):
    if record == "shmuel":
        b = u"Tiferet Shmuel on  " + masechet
