...
def open_file():
    with open("source/Tiferet_Shmuel_on_%s.txt" % masechet, 'r') as filep:
...
def save_parsed_text(text):
    ...
    with open("preprocess_json/Tiferet_Shmuel_on_%s.json" % masechet , 'w') as out:
        json.dump(text_whole, out)


def run_post_to_api():
   # Helper.createBookRecord(book_record())
    with open("preprocess_json/Tiferet_Shmuel_on_%s.json" % masechet, 'r') as filep:
        file_text = filep.read()
    Helper.postText( "Tiferet Shmuel on %s" % masechet, file_text, False)
