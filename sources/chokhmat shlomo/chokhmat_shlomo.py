...
for daf in dh1_dict:
	...
	for category, dh in dh1_dict[daf]:
		print category
		if category == 'rashi':
			if masechet == "Bava Batra" and daf > 57:
				continue
			maharam_line+=1
			rashi_line+=1
		...
		if category == 'rashi' or category == 'tosafot':
			...
			if out_order != 0: #out_order only equals 0 if there really is no Tosafot or Rashi on the given daf
				...
				if len(masechet_daf_line)>0:
					...
					"refs": [
								 masechet_daf_line,
								"Maharshal on "+masechet+"."+AddressTalmud.toStr("en", daf)+"."+str(maharam_line)
							],
					"type": "commentary",
					"auto": True,
				...
				if len(masechet_daf_line)>0:
					...
					post_link({
						...
						"type": "commentary",
						"auto": True,
						"generated_by": "Maharshal "+masechet+" linker"})
				...
				for key in mishnah_in_order[perek]:
					...
					if pos==mishnah_line:
						...
						post_link({
							"refs": [
									 masechet_daf_line,
									"Maharshal "+masechet+"."+AddressTalmud.toStr("en", daf)+"."+str(maharam_line)
								],
							"type": "commentary",
							"auto": True,
							"generated_by": "Maharshal on "+masechet+" linker",
