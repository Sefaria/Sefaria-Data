...
from rif_utils import tags_map, path, get_hebrew_masechet, main_mefaresh, commentaries, maor_tags
...
def post_mefareshim(masechtot=list(tags_map), mefarshim=[1,5,7,8,9], index=True, text=True, links=True, server = 'http://localhost:8000'):
        ...
        for masechet in masechtot:
            ...
            if masechet == 'intro':
                ...
                if masechet in library.get_indexes_in_category(["Talmud", "Bavli", "Seder Nashim"])+library.get_indexes_in_category(["Talmud", "Bavli", "Seder Nezikin"]):
                    godel, hgodel = 'HaGadol', 'הגדול'
                else:
                    godel, hgodel = 'HaKatan', 'הקטן'
            ...
            if index:
                ...
                functions.post_index(index_dict, server = server)
            if text:
                with open(path+f'/commentaries/json/{mefaresh}_{masechet}.json') as fp:
                    data = json.load(fp)
                text_version = {
                    'versionTitle': 'Vilna Edition',
                    'versionSource': "http://primo.nli.org.il/primo_library/libweb/action/dlDisplay.do?vid=NLI&docId=NNL_ALEPH001300957",
...
if __name__ == '__main__':
    ...
    #post_rif(text=True,server=server)
    #post_mefaresh(text=True,category=False,server=server)
    #post_mefareshim(server=server)
    post_maor(index=False,text=False)
