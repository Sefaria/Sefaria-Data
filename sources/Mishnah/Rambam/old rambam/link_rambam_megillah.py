...
title_book = "Mishnah Megillah"
title_comm = "Rambam on Mishnah Megillah" 
...
def getLog(hyperlink, perek, result):
	log = []
	count = 1
	for key in result:
		list = result[key]
		if list[0] == 0:
			log.append(hyperlink+"."+str(perek)+"."+str(count)+"\nNot Matched\n\n")
		elif len(list) > 1:
			guess_str = hyperlink+"."+str(perek)+"."+str(count)+"\nGuessed line "+str(list[0])+", but other options include line(s) "
			for guess in list:
				if guess != list[0]:
					guess_str += str(guess)+","
			...
			mishnah_n = result[key][0]
		  	if mishnah_n > 0:
				...
				if prev_comm != curr_comm or prev_mishnah_n != mishnah_n:
					...
					"language": "he",
					"text": " ",#comm_dict[(mishnah_n, curr_comm)]+curr_comm,
					}
					prev_comm = curr_comm
