...
def local_pipeline():
    ...
    for sec in sections:
        print(sec)
        r_string_comm = "Crescas on Guide for the Perplexed, {}".format(sec)
    ...
    for l in links:
        ...
        if "Crescas on Guide for the Perplexed, Part 1" in l["refs"][0] or "Crescas on Guide for the Perplexed, Part 2" in \
                l["refs"][0] or "Crescas on Guide for the Perplexed, Part 3" in l["refs"][0]:
            index = l["refs"][0].rfind(" ")

            # replace the last space with a colon
            l["refs"][0] = l["refs"][0][:index] + ":" + l["refs"][0][index + 1:]
    with open("crescas_dh_links.json", "w") as f:
...
def cauldron_pipeline():
    with open('crescas_dh_links.json') as f:
        links = json.load(f)
    links = list_of_dict_to_links(links)
    insert_links_to_db(links)
...
def clean():
    query = {"refs": {"$regex" : "Crescas on Guide for the Perplexed"}}
...
def create_report():
    query = {"refs": {"$regex": "Crescas on Guide for the Perplexed"}}
    list_of_links = LinkSet(query).array()
    list_of_tuples = [("Crescas", "Guide for the Perplexed")]
    for sec in sections:
        r_string_comm = "Crescas on Guide for the Perplexed, {}".format(sec)
        r_string_base = "Guide for the Perplexed, {}".format(sec)
    ...
    with open("crescas_report.csv", "w", newline="") as file:
