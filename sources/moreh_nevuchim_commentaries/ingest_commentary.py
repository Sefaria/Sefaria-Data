...
def create_text_object():
    ...
    # return [list(comm_list[0].values()), list(comm_list[1].values()),list(comm_list[2].values()), list(comm_list[3].values()), list(comm_list[4].values()), list(comm_list[5].values()), list(comm_list[6].values())]
...
def ingest_version(map_text):
    ...
    cur_version = VersionSet({'title': 'fodi on Guide for the Perplexed'})
    if cur_version.count() > 0:
        cur_version.delete()
