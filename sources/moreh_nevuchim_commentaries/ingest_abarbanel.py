...
def create_text_object():
    ref_prefix = "Abarbanel on Guide for the Perplexed, "
    sections = ["Introduction, Letter to R Joseph son of Judah", "Introduction, Prefatory Remarks",
                "Introduction, Introduction",
                "Part 1",  "Part 2",  "Part 3", "Strictures to Maimonides' Interpretation of the Account of the Chariot", "A Short Treatise on the Secrets of the Guide"]
    ref = ''
    ref_chapter = 0
    ref_paragraph = 0
    # Open the docx file
    document = Document('abarbanel.docx')
    ...
    strictures = {}
    treatise = {}
    comm_list = [letter_to_R_Joseph_son_of_Judah, prefatory_Remarks, introduction, part_1,  part_2, part_3, strictures, treatise]
...
def ingest_version(map_text):
    ...
    index = library.get_index("Abarbanel on Guide for the Perplexed")
    cur_version = VersionSet({'title': 'Abarbanel on Guide for the Perplexed'})
    ...
    version = Version({"versionTitle": "Warsaw, 1872",
                       ...
                       "title": "Abarbanel on Guide for the Perplexed",
                       "language": "he",
                       "chapter": chapter,
...
def ingest_nodes():
    ...
    en_title = 'Abarbanel on Guide for the Perplexed'
    he_title = 'אברבנאל על מורה נבוכים'
    root = SchemaNode()
    ...
    strictures_node = JaggedArrayNode()
    strictures_node.add_structure(["Paragraph"])
    strictures_node.key = "Strictures to Maimonides' Interpretation of the Account of the Chariot"
    strictures_node.add_primary_titles("Strictures to Maimonides' Interpretation of the Account of the Chariot", 'טענות נגד פירוש הרמב"ם במרכבת יחזקאל')

    treatise_node = JaggedArrayNode()
    treatise_node.add_structure(["Paragraph"])
    treatise_node.key = "A Short Treatise on the Secrets of the Guide"
    treatise_node.add_primary_titles("A Short Treatise on the Secrets of the Guide", 'באור סוד ספר מורה נבוכים')

    root.append(introduction_node)
    root.append(part1_node)
    root.append(part2_node)
    root.append(part3_node)
    root.append(strictures_node)
    root.append(treatise_node)
    root.validate()
