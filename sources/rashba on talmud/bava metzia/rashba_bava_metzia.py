...
for count_file in range(15):
	...
	for line in f:
		...
		if line.find("@") > 4:
			line = "@28 "+line
		if line.find("@28@60")>=0:
			line = line.replace("@28@60", "@60")
		elif line.find("@60@28")>=0:
			line = line.replace("@60@28", "")
		elif line.find("@70@28")>=0:
			line = line.replace("@70@28","@70")
		elif line.find("@28@70")>=0:
			line=line.replace("@28@70", "@70")
		elif line.find("@28@20")>=0:
			line=line.replace("@28@20", "@20")
		line = line.replace("@14", "")
		line = line.replace("@04", "")
		if line.find("@40") != -1 and line.rfind("@40") != -1 and line.find("@40") != line.rfind("@40"):
...
for daf in dh_dict.keys():
	if len(dh_dict[daf]) != len(comm_dict[daf]):
		pdb.set_trace()
for daf in dh_dict.keys():
	text = get_text("Bava Metzia."+AddressTalmud.toStr("en", daf))
	try:
	...
	for i in range(len(comm_dict[daf])):
		 if len(dh_dict[daf][i]) > 0:
			...
			post_link({
					"refs": [
							"Bava Metzia"+"."+AddressTalmud.toStr("en", daf)+"."+str(line_n), 
							"Rashba on Bava Metzia."+AddressTalmud.toStr("en", daf)+"."+str(i+1)
						],
					"type": "commentary",
					"auto": True,
					"generated_by": "Rashba on Bava Metzia linker",
				 })
	send_text = {
				"versionTitle": "Rashba on Bava Metzia",
				"versionSource": "http://www.sefaria.org",
				"language": "he",
				"text": comments,
				}
	
	post_text("Rashba on Bava Metzia."+AddressTalmud.toStr("en", daf), send_text)
	pdb.set_trace()
