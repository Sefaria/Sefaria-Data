...
def post_text(ref, text):
    ...
    try:
        ...
        x=response.read()
        if x.find("error")>=0 and x.find("Line")>=0:
        	pdb.set_trace()
    except HTTPError, e:
...
def removeSecond40(temp_text):
	...
	for i in range(len(arr)):
		if found_40 == False or arr[i].find("@40")==-1:
			...
			new_temp_text += arr[i].replace("@30","").replace("@40","") + " "		
		if arr[i].find("@40") >= 0:
			found_40 = True
...
for count_file in range(10):
	...
	for line in f:
		...
		if temp_text.find("@") > 4:
			if temp_text.find("@30") or temp_text.find("@70"):
		...
		if (temp_text.find("@30") >= 0 or temp_text.find("@70") >= 0) and temp_text.rfind("@40")>=0:
			...
			if daf in dh_dict:
				...
				dh_dict[daf].append(dh)
			just_added_dh = True
		elif temp_text.find("@30") >= 0:
			pdb.set_trace()
		if temp_text.find("@40") >= 0 and (temp_text.find("@60") == -1 or temp_text.rfind("@60") > 2):
			start = temp_text.find("@40")
			end = max(temp_text.find("@10"), temp_text.rfind("@60"), temp_text.find("@50"))
			if end == -1:
...
for daf in dh_dict.keys():
	if len(dh_dict[daf]) != len(comm_dict[daf]):
		pdb.set_trace()
for daf in dh_dict.keys():
	text = get_text("Nedarim."+AddressTalmud.toStr("en", daf))
	try:
	...
	for i in range(len(comm_dict[daf])):
		 if len(dh_dict[daf][i]) > 0:
			...
			post_link({
					"refs": [
							"Nedarim"+"."+AddressTalmud.toStr("en", daf)+"."+str(line_n), 
							"Rashba on Nedarim."+AddressTalmud.toStr("en", daf)+"."+str(i+1)
						],
					"type": "commentary",
					"auto": True,
					"generated_by": "Rashba on Nedarim linker",
				 })
	send_text = {
				"versionTitle": "Rashba on Nedarim",
				"versionSource": "http://www.sefaria.org",
				"language": "he",
				"text": comments,
				}
	
	post_text("Rashba on Nedarim."+AddressTalmud.toStr("en", daf), send_text)
	pdb.set_trace()
