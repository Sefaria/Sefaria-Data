...
def replaceBadTags(temp_text):
	temp_text = temp_text.replace("@98@77@20", "@20")
	temp_text = temp_text.replace("@98@77@30", "@30")
	temp_text = temp_text.replace("@98@77", "@60")
	temp_text = temp_text.replace("@77@30", "@30")
	temp_text = temp_text.replace("@77@20", "@20")
	temp_text = temp_text.replace("@77@70", "@70")
	temp_text = temp_text.replace("@77", "@60")
	temp_text = temp_text.replace("@98", "@60")
...
for count_file in range(8):
	lines = []
	f = open(masechet+"/"+title+str(title_option+count_file)+".txt")
	for line in f:
		line=line.replace("\n","")
...
for daf in dh_dict.keys():
	...
	for i in range(len(comm_dict[daf])):
		if len(dh_dict[daf][i])>0:
			if comm_dict[daf][i].find("]")>=0 or comm_dict[daf][i].find(")")>=0:
				has_brackets = True
			comments.append(comm_dict[daf][i])
			
	if has_brackets:
		brackets_file.write(AddressTalmud.toStr("en",daf)+"\n")
	send_text = {
