	...
	for line in f:
		...
		if temp_text.find("@22") >= 0:
			...
			if daf in dh_dict:
				...
				dh_dict[daf].append(dh)
			just_added_dh = True
		if temp_text.rfind("@11") >= 0 and (temp_text.find("@33") == -1 or temp_text.find("@33") > 2):
...
for daf in dh_dict.keys():
	...
	for i in range(len(comm_dict[daf])):
		 if len(dh_dict[daf][i]) > 0:
			...
			post_link({
					"refs": [
							"Beitzah"+"."+AddressTalmud.toStr("en", daf)+"."+str(line_n), 
							"Rashba on Beitzah."+AddressTalmud.toStr("en", daf)+"."+str(i+1)
						],
					"type": "commentary",
					"auto": True,
					"generated_by": "Rashba on Beitzah linker",
				 })
	send_text = {
				"versionTitle": "Rashba on Beitzah",
				"versionSource": "http://www.sefaria.org",
				"language": "he",
				"text": comments,
				}
	
	post_text("Rashba on Beitzah."+AddressTalmud.toStr("en", daf), send_text)
	pdb.set_trace()
