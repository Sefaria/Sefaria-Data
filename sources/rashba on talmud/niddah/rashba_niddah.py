...
for daf in dh_dict.keys():
	...
	for i in range(len(comm_dict[daf])):
		 if len(dh_dict[daf][i]) > 0:
			...
			post_link({
					"refs": [
							"Niddah"+"."+AddressTalmud.toStr("en", daf)+"."+str(line_n), 
							"Rashba on Niddah."+AddressTalmud.toStr("en", daf)+"."+str(i+1)
						],
					"type": "commentary",
					"auto": True,
					"generated_by": "Rashba on Niddah linker",
				 })
	send_text = {
				"versionTitle": "Rashba on Niddah",
				"versionSource": "http://www.sefaria.org",
				"language": "he",
				"text": comments,
				}
	
	post_text("Rashba on Niddah."+AddressTalmud.toStr("en", daf), send_text)
	pdb.set_trace()
