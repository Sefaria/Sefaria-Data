...
for daf in dh_dict.keys():
	if len(dh_dict[daf]) != len(comm_dict[daf]):
		pdb.set_trace()
for daf in dh_dict.keys():
	text = get_text("Kiddushin."+AddressTalmud.toStr("en", daf))
	try:
	...
	for i in range(len(comm_dict[daf])):
		 if len(dh_dict[daf][i]) > 0:
			...
			post_link({
					"refs": [
							"Kiddushin"+"."+AddressTalmud.toStr("en", daf)+"."+str(line_n), 
							"Rashba on Kiddushin."+AddressTalmud.toStr("en", daf)+"."+str(i+1)
						],
					"type": "commentary",
					"auto": True,
					"generated_by": "Rashba on Kiddushin linker",
				 })
	send_text = {
				"versionTitle": "Rashba on Kiddushin",
				"versionSource": "http://www.sefaria.org",
				"language": "he",
				"text": comments,
				}
	
	post_text("Rashba on Kiddushin."+AddressTalmud.toStr("en", daf), send_text)
	pdb.set_trace()
