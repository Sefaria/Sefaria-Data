...
def unite_refs(refs):
    ...
    for title in titles:
        tit_refs = [ref.split()[-1] for ref in refs if title == ' '.join(ref.split()[:-1])]
...
def may_gemartia(input):
    if type(input) == int:
        return input
    else:
        return getGematria(input)

def map_zucker(mas, chapter, halakha, data):
    if mas == 'עדויות':
        mas = 'עדיות'
    if halakha:
        nums = [e['num'] for e in data if e['tractate']==mas and e['chapter'] == int(chapter) and may_gemartia(e['halakha']) == int(halakha)]
    else:
...
def create_manusripts():
    ...
    m.he_description = ''
    f = []
    for t, ht in [['Erfurt Manuscript', 'כתב יד ארפורט'], ['Vienna Manuscript', 'כתב יד וינה']]:
        m.title = t
        m.he_title = ht
        f.append(m.contents())
        try:
...
def find_vienna_file_from_num(vienna_filenames, num, vienna):
    entries = [entry for entry in vienna if int(entry['num']) == num]
    for entry in entries:
        mas = re.sub('Mishnah |Bavli ', '', Ref(entry['tractate']).normal()).replace('ata', 'atta')
        new = set(find_vienna(vienna_filenames[mas], entry['chapter'], getGematria(entry['halakha'])))
        try:
...
def parse_new_csvs():
    skip = False
    vienna, erfurt = [], []
    for data in [vienna, erfurt]:
        ms = 'vienna' if data == vienna else 'erfurt'
        page = 4 if data == vienna else 3
        with open(f'{ms}_lieb.csv', encoding='utf-8', newline='') as fp:
            for row in csv.DictReader(fp):
                if row['tractate']:
                    trac = re.sub('תוספתא|\(ליברמן\)', '', row['tractate']).strip()
                    chapter = 0
                if row['chapter']:
                    new = int(row['chapter'].replace('פרק', ''))
                    if new != chapter + 1:
                        print(f'sequence problem in {trac}. ch. {new} comes after {chapter}')
                    chapter = new
                    halak = 1
                if row['content']:
                    if not skip:
                        data.append({'tractate': trac, 'chapter': chapter, 'halakha': halak, 'num': page})
                    if 'L' in row['content']:
                        if not row['content'].strip().endswith('L'):
                            print(f'L in the middle {row["content"]}')
                        halak += 1
                if row['page']:
                    if row['page'] != '@':
                        print(f'{row["page"]} in the column of @s')
                        continue
                    page += 1
                    if page == 215 and data == vienna:
                        page += 1
                    if not row['content']:
                        skip = True
                    if skip and row['content']:
                        skip = False
                        page -= 1
    return vienna, erfurt

def vilna_to_lieb(chapter, halakha, masechet):
    hal = f'Tosefta {mas} {chapter}:{halakha}'
    with open(f'/home/yishai/sefaria/tosefta/tosefta/source_files/{masechet.replace(" ", "_").replace("attan", "atan")}/mapping.json') as fp:
        data = json.load(fp)
    for key in data:
        Ref(data[key])
    return [key.split()[-1].split(':') for key in data if Ref(hal) in Ref(data[key]).all_segment_refs()]
...
if __name__ == '__main__':
    ...
    vienna_lieb, erfurt_lieb = parse_new_csvs()
        ...
        for ind in inds:
            ...
            if not lieb:
                ...
                for ref in Ref(ind).all_segment_refs() + Ref(f'{ind} (Lieberman)'.replace('Kattan', 'Katan')).all_segment_refs():
                    ...
                    if 'Lieberman' not in ref.tref:
                        new = vilna_to_lieb(chapter, halakha, mas)
                    else:
                        new = [(chapter, halakha)]
                    for chapter, halakha in new:
                        nums = map_zucker(he_mas, chapter, halakha, erfurt_lieb)
                        erfurt_images = add_refs(erfurt_images, nums, ref)
                        vienna_nums = map_zucker(he_mas, chapter, halakha, vienna_lieb)
                        vienna_addres = []
                        for num in vienna_nums:
                            vienna_addres += (find_vienna_file_from_num(vienna_filenames, num, vienna))
                        vienna_images = add_refs(vienna_images, vienna_addres, ref)
    ...
    for images in [erfurt_images, vienna_images]:
        for num, refs in images.items():
            ...
            if f_name:
                mp.manuscript_slug = 'vienna-manuscript'
            else:
                mp.manuscript_slug = "erfurt-manuscript"
            if f_name:
                if num < 642:
