...
def base_tokenizer(some_string):
    return filter(lambda(x): x!=u'',remove_extra_space(some_string.replace(u"<b>",u"").replace(u"</b>",u"").replace(".","").replace(u"\n",u"")).split(u" "))
def m_post_index():
    # create index record
    record = SchemaNode()
...
def write_missed_comments(comment_list):
    f = open("Mizrachi Missed Comment Report.txt","a")
...
if posting_index:
    m_post_index()
f = open("Mizrachi Missed Comment Report.txt","w")
f.close()
