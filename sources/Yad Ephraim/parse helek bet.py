...
def combine_files():
    with open("aleph.txt") as a:
        with open("bet.txt") as b:
            with open("gimmel.txt") as g:
                lines = list(a) + list(b) + list(g)
                with open("yad_ephraim.txt", 'w') as new_file:
                    new_file.writelines(lines)
if __name__ == "__main__":
    simanim_f = open("simanim.csv", 'w')
    ...
    prev = 1
    prev_line = ""
    with open("yad_ephraim.txt") as f:
        lines = list(f)
        for n, line in enumerate(lines):
            if line == "\n":
                continue
            seif_k = re.search("@11(.*?)@33", line)
            words = line.split()
            siman = None
            ...
            if seif_k:
                seif_k = seif_k.group(1)
                if len(seif_k.split()) == 3:
                    book = seif_k.split()[0]
                    seif_katan = seif_k.split()[-1]
                    #print "{} -> {}".format(book, seif_k)
                    book_set.add(book)
                    prev_book = book
                elif len(seif_k.split()) == 2:
                    book = prev_book
                    seif_katan = seif_k.split()[-1]
                else:
                    pass
