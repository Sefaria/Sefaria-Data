...
def load_raw_from_sheet():
    f = './Background entries DATABASE - live - Background entries DATABASE - live.csv'
...
def interlink_topics(orig_descs):
    ...
    for topix in get_ashlag_topics():
        ...
        def do_sub(m, group=0):
            ...
            norm = normalize(remove_tags(rawmatch)).replace("*", "")
...
def wrap_all():
    ...
    mapping = get_normal_mapping()
    for t in TEXTS:
        old_create_topic_links_for_text(t, mapping)


def old_create_topic_links_for_text(text_name, mapping):
...
def create_topic_links_for_text(text_name, mapping):
    ...
    for v in vs:
        ...
        for ref in Ref(text_name).all_segment_refs():
            ...
            # Find and record all matching terms
            for referral in re.findall("<i>.*?</i>", tc.text):
                norm = normalize(remove_tags(referral))
                topix = mapping.get(norm)
                if topix:
                    matches.add_match(ref, referral, topix)
...
def change_markers_on_new_texts(texts):
    pattern = r'\{([^{}]*)\}'

    # Define a function to be used for the substitution
    def replace_brackets(match):
        return f"<<{match.group(1)}>>"
    for t in texts:
        for ref in library.get_index(t).all_segment_refs():
            tc = TextChunk(ref, lang='en', vtitle='The Sefaria Sulam, 2023')
            # if "<<" in tc.text:
            #     print(tc.text)
            #     print(ref)
            new_text = re.sub(pattern, replace_brackets, tc.text)
            if tc.text != new_text:
                print(ref)
                tc.text = new_text
                tc.save()
...
#change_markers_on_new_texts(["Talmud Eser HaSefirot","Sulam on Zohar"])
print(RefTopicLinkSet({"dataSource": SOURCE_SLUG}).count())
print(IntraTopicLinkSet({"dataSource": SOURCE_SLUG}).count())
#test_coverage()
build_all_topics()
