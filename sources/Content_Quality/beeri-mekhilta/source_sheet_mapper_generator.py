...
def rewriter_function(prod_ref, mapper, uuid=None):
    ...
    elif prod_ref and not Ref(prod_ref).is_segment_level():
        print(f"{prod_ref} needs to be handled manually for UUID {uuid}")
        return ""

    # If a section or book level ref, determine the mapping based on the segment refs within
    elif prod_ref and Ref(prod_ref).is_range():
...
if __name__ == '__main__':
    ...
    for document in result:
        uuid = document["owner"]
        mapper_dict = {"uuid": uuid, "includedRefsMap": [], "expandedRefsMap": []}

        for ref in document["includedRefs"]:
            if "Mekhilta d'Rabbi Yishmael" in ref:
                mapper_dict["includedRefsMap"].append({"old_ref": ref, "new_ref": rewriter_function(ref, mapping, uuid)})

        for ref in document["expandedRefs"]:
            if "Mekhilta d'Rabbi Yishmael" in ref:
                mapped_ref = rewriter_function(ref, mapping, uuid)
                if mapped_ref:
