...
def replace_girsa_letters(instring):
    return re.sub(r'\s*([A-Z])(\s+)', r'<sup>\1</sup>\2', instring)

def find_letters(instring):
    return re.findall(r"\s*([A-Z])(\s+)", instring)
...
for index in jtindxes:
    ...
    for halacha in all_halachot:
        ...
        new  = [replace_footnote(replace_girsa_letters(t)) for t in tc.text]
