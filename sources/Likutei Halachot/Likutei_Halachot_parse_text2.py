...
def extract_text(file_input):
    with codecs.open(file_input, "r", "utf-8") as myfile:
...
def extract_title(s):
    #fix minor glitches:
    s.replace(u"ענין‏","")
    if  u"פרשיות" in s and u"ארבע" in s:
        return s.strip().split(" ")
    if u"הקדמת‏" in s and u"המחבר" in s:
        return s.strip().split(" ")
...
#it's easier to fix that way (we can run a blanket replace on דינים since this is likely found in other contexts)
    if u"דיינים" in s:
        s.replace(u"דיינים", u"דינים")
    return (s.strip()).split(" ")[:-1]
#this method spot-fixes glitches in the titles

def is_text_segment(s):
    non_text = [u"בס\"ד", u"‏ליקוטי‏ ‏הלכות‏"  ,u"מתוך אוצר ספרי ברסלב להורדה של אור הגנוז ברסלב", "orhaganuz"]
