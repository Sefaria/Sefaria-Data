...
def extract_text(file_input):
    ...
    for index, line in enumerate(text):
        ...
        if not_blank(line) and is_text_segment(line):
            ...
            if True!=wordHasNekudot(line) and len(line)<200 and no_ignore(line):
                ...
                if ("‏אות" not in line and len(line)>17) or "@@BREAK" in line:
                    section_box.append(ois_box)
                    ois_box=[]
                    #to distinuish new topics from new sections, we keep a log of topics already parsed
                    title = extract_title(line)
                    if title not in parsed_topics and not_blank(title) and not_marker(title):
            ...
            #we sdon't want to append names of sections, ois's, or topics, so we only append if our line is none of these, hence "else"
                ...
                paragraph_box.append(unmark(line))
...
def get_parsed_text():
    ...
    for indexo, order in enumerate(order_files):
        for file_name in order:
            ...
            for topics in file_parse_return[1]:
                for topic in topics:
...
def is_text_segment(s):
    ...
    for entry in non_text:
        if entry in s:
            return False
    return True;
#IGNORE tags are lines without nikudot that are not section headers
def no_ignore(s):
