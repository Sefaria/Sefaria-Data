        ...
        if not_blank(line):
            if re.search(ur'^(הקדמה|השלמה|חתימת|חלק|פתיחה לע)',line):
                ...
                if re.search(ur'^(דיני מצות הלואה|דין עניני העבוט|דיני תשלומי שכר שכיר בזמנו)',line):
                    goes_in_next.append(u'<b>'+clean_ac_line(line)+u'</b><br>')
                    ...
                    goes_in_next.append(u'<b>'+clean_nc_line(line)+u'</b>')
                elif re.search(ur'^{} '.format(perek_string),line):
                    if len(goes_in_next)>0:
                        goes_in_next[-1]=goes_in_next[-1]+u'<b>'+clean_ac_line(line)+u'</b>'
                    else:
                        goes_in_next.append(u'<b>'+clean_nc_line(line)+u'</b>')
                    ...
                    while len(goes_in_next)>0:
                        print "BEFORE", post_line
                        post_line=goes_in_next.pop()+u'<br>'+post_line
                        print "AFTER", post_line
                    if next_goes_back:
                        next_goes_back=False
                        if len(ac_chapter_box)>0:
                            ac_chapter_box[-1]=ac_chapter_box[-1]+u'<br>'+clean_ac_line(post_line)
                        else:
                            ac_chapter_box.append(clean_ac_line(post_line))
                    else:
                        ac_chapter_box.append(clean_ac_line(post_line))
            else:
