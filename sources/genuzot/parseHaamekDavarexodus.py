...
def book_record():
    return {
        "title": "Haamek Davar on Exodus",
        "titleVariants": ["hanatziv on Exodus", "Haamek Davar on Shmot"],
        "heTitle": 'העמק דבר שמות',
        "heTitleVariants" :['הנצי"ב על שמות', 'העמק דבר'],
        "sectionNames": ["", ""],
        "categories": ['Commentary'],
    }
def open_file():
    with open("source/haamek_davar_exodus.txt", 'r') as filep:
...
def save_parsed_text(text):
    ...
    with open("preprocess_json/Haamek_Davar_on_Exodus.json", 'w') as out:
