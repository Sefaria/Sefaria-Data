    ...
    if lex_lookup:
        ...
        for i, lookup in enumerate(lex_lookup):
            csv_row = {
                ...
                "URL" : "https://www.sefaria.org/{}, {}".format(lookup["parent_lexicon_details"]["index_title"], lookup["headword"]) if "index_title" in lookup["parent_lexicon_details"] else ""
            }
...
with open("/tmp/most_common_words_and_lexicon_entries.csv", "wb") as fout:
    writer = csv.DictWriter(fout, ["Common Word", "Headword", "Lexicon", "URL"])
    writer.writeheader()
