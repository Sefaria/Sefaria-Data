...
with open("most_common_words_and_lexicon_entries_revised.csv") as csvfile:
    ...
    for i, row in enumerate(reader, 2):
        ...
        print u"{}) {}-{}-{} [p{}, a{}, d{}]".format(i, word_form_str, headword, lexicon, primaryH, addH, deleteH)
        query = { "$or": [{"c_form": word_form_str}, {"form": word_form_str}],
                  "lookups": { "$elemMatch": { "headword": headword, "parent_lexicon": lexicon } } }
        if addH or primaryH:
            print u"{}-{}-{} being added ({} {})".format(word_form_str, headword, lexicon, "New" if addH else "", "Primary" if primaryH else "")
            form = create_Word_form(word_form_str, headword, lexicon, primaryH)
        elif deleteH:
            print u"{}-{}-{} being deleted:".format(word_form_str, headword, lexicon)
            wfset = WordFormSet(query)
            for wf in wfset:
                len_lookups = len(wf.lookups)
                wf.lookups = filter(lambda x: x["headword"] != headword or x["parent_lexicon"] != lexicon, wf.lookups)
                len_lookups_adj = len(wf.lookups)
                print u"*     Original lookups: {} | Lookups removed {}".format(len_lookups, len_lookups-len_lookups_adj)
                if len_lookups_adj > 0:
                    print u"*     saving word form"
                    try:
                        wf.save()
                    except:
                        pass
                else:
                    print u"*     deleting word form"
                    try:
                        wf.delete()
                    except:
                        pass
