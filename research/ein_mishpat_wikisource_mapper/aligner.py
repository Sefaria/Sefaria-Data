...
for mes in mesechtot:
    ...
    for daf_ref in mes_ind.all_section_refs():
        ...
        word_start_list = []
        matched_iter = re.finditer(r"\s+([^ ])", daf_scraped["text"])
        ...
        while curr_word < len(words_scraped):
            possible_range = list(range(curr_word, curr_word+chunk_size))
            if curr_super_word < len(super_word_list) and super_word_list[curr_super_word] - 1 in possible_range:
                comments_scraped += [" ".join(words_scraped[curr_word:super_word_list[curr_super_word]])]
                ...
                comments_scraped += [" ".join(words_scraped[curr_word:curr_word + temp_chunk])]
                super_comment_list += [len(comments_scraped)-1]
                curr_word += temp_chunk
                curr_super_word += 1
            else:
                comments_scraped += [" ".join(words_scraped[curr_word:curr_word + chunk_size])]
        ...
        wiki_snippets = [" <{}> ".format(bleach.clean(sim, strip=True, tags=[])).join([comments_scraped[sc-1],
                         comments_scraped[sc]]) if sc != 0 else " <{}> {}".format(bleach.clean(sim, strip=True, tags=[]
                         ), comments_scraped[sc]) for sim, sc in zip(daf_scraped["super_simanim"], super_comment_list)]
        out_rows += [
            {"Index": ind,
             "Daf": daf_ref.normal(),
             "Siman": bleach.clean(sim, strip=True, tags=[]),
             "Siman Ref": Ref(sim_ref).starting_ref().normal() if sim_ref is not None else "N/A",
             "Wiki Snippet": ws
             } for ind, sim, sim_ref, ws in zip(range(len(out_rows)+1, len(out_rows)+1+len(super_sefaria_list)),
