...
def add_intro_node(record):
    """
    Modifies the SchemaNode record for the Index by adding a node to contain the text for the
    Introduction of a text.
    :param record: SchemaNode for the Index
    :return: None. Modifies the SchemaNode record in-place.
    """

    # Create the JaggedArray node
    intro_node = JaggedArrayNode()

    # Add the English title as a primary title, with the corresponding langauge code "en"
    intro_node.add_title("Introduction", 'en', primary=True)

    # Add the Hebrew title as a primary title, with the corresponding langauge code "he"
    intro_node.add_title("הקדמה", 'he', primary=True)

    # Add the key, "Introduction"
    intro_node.key = "Introduction"

    # Set the depth, the addressTypes and the sectionNames. In this case, since the text is of depth-1, the addressTypes
    # and sectionNames will be lists of length 1.
    intro_node.depth = 1
    intro_node.addressTypes = ['Integer']
    intro_node.sectionNames = ['Paragraph']

    # Append the JaggedArray node for the Introduction to the parent SchemaNode record.
...
def add_text_node(record):
    ...
    # Set the key to "default", and the value of the default field to True
    # (When a Schema has an Intro, and a Text node, the value of the default field must be
    # set to True, and the key must be 'default').
...
def create_index_main():
    ...
    for mishnah_index in mishnayot:
        ...
        # Add the Introduction node to the SchemaNode record
        add_intro_node(record)

        # Add the Text node to the SchemaNode record
